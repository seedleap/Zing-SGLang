...
class MinWMVideoArchConfig(DiTArchConfig):
    ...
    def __post_init__(self):
        ...
        if self.action_type not in {
            ...
            raise ValueError(
                "Zing action_type must be primitive_token_residual or "
                "primitive_rope_token_residual"
            )
        if self.rope_position_mode not in {"absolute", "block_relative"}:
            raise ValueError(
                "Zing rope_position_mode must be absolute or block_relative"
            )
        ...
        if self.local_attn_size != -1 and self.local_attn_size <= 0:
            raise ValueError("Zing local_attn_size must be -1 or positive")
        if self.sink_size < 0:
            raise ValueError("Zing sink_size must be non-negative")
        ...
        if (
            ...
            raise ValueError(
                "Zing sliding_window_num_frames must equal bounded local_attn_size"
            )
        if self.rope_position_mode == "block_relative" and self.scene_cut_rope_offset:
            raise ValueError(
                "Zing block_relative RoPE does not support scene_cut_rope_offset"
            )
