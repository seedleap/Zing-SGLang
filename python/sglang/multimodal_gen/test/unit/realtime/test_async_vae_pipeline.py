...
def test_remote_frame_handlers_keep_first_frame_trace_scoped_to_their_chunk():
    async def scenario():
        ...
        session = realtime_video_api.GenerateSession()
        ...
        first_frame_traces = [
            ...
            if event == "server.remote_first_frame_received"
        ]
...
def test_async_vae_open_failure_still_closes_client(monkeypatch):
    async def scenario():
        ...
        session = realtime_video_api.GenerateSession(session_id="s", generation_id="g")
