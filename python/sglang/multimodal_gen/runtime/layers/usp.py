    ...
    *,
    ...
    if output_buffer is None:
        output = (
            torch.empty_like(x)
            if role is None
            else _a2a_staging_buffer(role, x.shape, x.dtype, x.device)
        )
    else:
        if (
...
def _usp_pack_peer_first_qkv(
    ...
    if (
        ...
        try:
            from sglang.kernels.ops.diffusion.norm.ulysses_qkv_pack import (
...
def _usp_input_all_to_all_peer_first_qkv(
    query: torch.Tensor,
    key: torch.Tensor,
    value: torch.Tensor,
    *,
...
def _usp_input_all_to_all_varlen_qkv(
    ...
    """Varlen counterpart of :func:`_usp_input_all_to_all_peer_first_qkv`."""
    world_size = get_ulysses_parallel_world_size()
    if world_size <= 1:
        return torch.cat((query, key, value), dim=-1)
    if query.shape != key.shape or query.shape != value.shape:
...
def _usp_output_all_to_all(
    ...
    x = x.permute(permute_order).contiguous()
    x = _usp_all_to_all_single(x, role="usp_output", output_buffer=output_buffer)
