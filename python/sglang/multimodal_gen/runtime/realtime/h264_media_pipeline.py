...
# Bound final draining. close() must send queued frames and the completion marker
# before tearing down the encoder, but a stuck ffmpeg must not hang cleanup.
...
@dataclass(frozen=True, slots=True)
class _DrainMarker:
    """FIFO sentinel proving all preceding queue entries were consumed."""
...
class H264MediaPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        # Final completion is the gateway's media barrier. Send it only after
        # ffmpeg stdout reaches EOF or the gateway may close before tail frames.
        self._pending_final_completion: _ChunkCompletion | None = None
    ...
    async def close(self) -> None:
        ...
        try:
            ...
            if failure is None:
                ...
                logger.warning(
                    "VAE H.264 encoder cleanup also failed: session_id=%s "
                    "generation_id=%s",
                    self.session_id,
                    self.generation_id,
                    exc_info=exc,
        ...
        try:
            # The final marker means media is complete only after the FIFO queue,
            # ffmpeg stdout, and the process all drain successfully.
            if failure is None and self._pending_final_completion is not None:
                await self._send_completion(self._pending_final_completion)
                self._pending_final_completion = None
        finally:
            self._queue.clear()
            self._queued_frames = 0
    ...
    async def _drain_pending(self) -> None:
        """Drain queued frames and completion markers before teardown.

        The final ``media_chunk_complete`` can be queued immediately before the
        session closes. Cancelling and clearing here would drop both tail frames
        and that marker, leaving the gateway waiting until its output-drain
        timeout and misclassifying a completed generation as a lifetime expiry.
        """
        if self._runner.done():
            self._raise_if_failed()
            raise RuntimeError("VAE H.264 pipeline stopped before final drain")
        ...
        try:
            ...
            raise RuntimeError("VAE H.264 pipeline drain timed out") from None
            ...
            while (item := await self._next()) is not None:
                ...
                if isinstance(item, _ChunkCompletion):
                    if item.is_final_chunk:
                        if self._pending_final_completion is not None:
                            raise ProtocolViolation(
                                "duplicate final H.264 chunk completion"
                            )
                        # The final completion cannot precede encoder EOF. close()
                        # drains stdin/stdout, then emits it as the media barrier.
                        self._pending_final_completion = item
                    else:
    ...
    async def _stop_ffmpeg(self) -> None:
        ...
        if process is not None:
            if process.stdin is not None:
                with contextlib.suppress(Exception):
                    process.stdin.close()
            # Closing stdin flushes buffered frames. Only a clean stdout EOF and
            # zero process status allow the caller to emit the final marker.
            try:
                ...
                return_code = await asyncio.wait_for(
                    process.wait(), timeout=CLOSE_DRAIN_TIMEOUT_S
                )
                if return_code != 0:
                    raise RuntimeError(
                        f"H.264 encoder exited with status {return_code}"
                    )
            except BaseException as exc:
                failure = exc
                with contextlib.suppress(Exception):
        ...
        if failure is not None:
            if isinstance(failure, TimeoutError):
                raise RuntimeError("H.264 encoder drain timed out") from failure
            raise failure
